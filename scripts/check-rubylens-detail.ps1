...
foreach ($path in @("metadata/domain-review.json", "config/review-profile.json", "fixtures/domain_review.csv", "docs/field-notes.md", "docs/review-journal.md", "examples/rubylens-walkthrough.md")) {
...
if ($profile.highest_case.name -ne "baseline") { throw "$repo profile highest case changed" }
if ($profile.lowest_case.name -ne "stale") { throw "$repo profile lowest case changed" }
