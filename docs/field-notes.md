# Field Notes

The fixture is small on purpose, which makes each domain case carry real weight.

The domain cases cover `rule hit`, `source span`, `risk density`, and `false positive cost`. They sit beside the smaller starter fixture so the project has both a compact scoring check and a domain-flavored review check.

The model rewards signal and confidence, then pulls back for drag. In this fixture, `stale` shows where that pullback matters most.

The language-specific addition keeps the review model in a module with Minitest coverage.
